...
using FunctionalAllocator.Extensions;
...
namespace FunctionalAllocator.ViewModels
{
    public abstract class ViewModelBase : INotifyPropertyChanged, INotifyDataErrorInfo
    {
        ...
        /// <summary>
        /// Validate and update value of a property
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="visualValueStorage"></param>
        /// <param name="value"></param>
        /// <param name="propertyName"></param>
        /// <param name="realValueStorage"></param>
        /// <returns></returns>
        protected virtual bool CheckAndSetNumericProperty<T>(ref T realValueStorage, ref string visualValueStorage, string value,
            [CallerMemberName] string propertyName = null) where T : IConvertible
        {
            if (Equals(visualValueStorage, value))
                return false;

            visualValueStorage = value;

            if (value == null)
            {
                SetPropertyError("Empty field.", propertyName);
                return false;
            }
            try
            {
                realValueStorage = value.ConvertToValue<T>();
                RemovePropertyError(propertyName);
                return true;
            }
            catch
            {
                SetPropertyError("Invalid value.", propertyName);
                return false;
            }
        }
        ...
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="setRealValueStorage"></param>
        /// <param name="visualValueStorage"></param>
        /// <param name="value"></param>
        /// <param name="propertyName"></param>
        /// <returns></returns>
        protected virtual bool CheckAndSetNumericProperty<T>(Action<T> setRealValueStorage, ref string visualValueStorage, string value,
            [CallerMemberName] string propertyName = null) where T : IConvertible
        {
            if (Equals(visualValueStorage, value))
                return false;
            ...
            try
            {
                setRealValueStorage(value.ConvertToValue<T>());
